...
v5 rewrites `src/eda` to match `deepsight-altair-software`'s `src/eda` millimeter-for-millimeter
(the static port registry addressed by `app::PortList` id, `StateMachine::change_state()`/
`return_to_last_state()`/two-phase `set_next_state()`, and the `IdleHook` module altair has and
this project previously did not), and introduces `hal/os` beneath it: a seam that wraps every
kernel primitive `eda/` uses (thread, queue, timer, idle callback) so `eda/` never includes a
kernel header directly. `eda/` and everything above it is now written against `hal::os`, not
against Zephyr, which is what makes a future move back to FreeRTOS a new `hal/os/freertos/`
backend instead of a rewrite of `eda/`. See §7's new `eda` / `hal/os` entry and
`src/eda/eda.md` / `src/hal/os/os.md` for the full rationale and the deviations from altair's
...
- **Why it exists**: `eda/` is ported from `deepsight-altair-software`, which is FreeRTOS
...
8. `eda::IdleHook` has no callback registered anywhere in this firmware today; it exists because it
   was asked for, matching `deepsight-altair-software`. If nothing ever registers one, it's a
