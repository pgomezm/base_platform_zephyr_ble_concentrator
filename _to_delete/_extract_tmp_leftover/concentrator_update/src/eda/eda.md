...
ported millimeter-for-millimeter from `deepsight-altair-software`'s `src/eda`, including the parts
that only make sense once other modules exist to use them (`Port::set_event_callback()`,
`StateMachine::return_to_last_state()`). The only thing that changed going from altair to this
...
or pointer to it. This is `deepsight-altair-software`'s design, carried over unchanged: it means a
...
## Known, deliberate deviations from deepsight-altair-software
