...
constexpr uint16_t k_max_devices = CONFIG_APP_MAX_DEVICES;
...
constexpr uint16_t k_adv_report_pool_size = CONFIG_APP_ADV_REPORT_POOL_SIZE;
...
constexpr uint16_t k_expected_company_id = CONFIG_APP_EXPECTED_COMPANY_ID;

/// Period between uplink dispatches, in minutes.
constexpr uint32_t k_dispatch_period_min = CONFIG_APP_DISPATCH_PERIOD_MIN;

/// Period between uplink dispatches, in milliseconds.
constexpr uint32_t k_dispatch_period_ms = k_dispatch_period_min * 60U * 1000U;
...
constexpr uint32_t k_device_stale_after_s = CONFIG_APP_DEVICE_STALE_AFTER_S;

/// Identifier of this concentrator, carried in every uplink header.
constexpr uint32_t k_concentrator_id = CONFIG_APP_CONCENTRATOR_ID;
...
constexpr bool k_report_boot_in_first_uplink = IS_ENABLED(CONFIG_APP_REPORT_BOOT);
