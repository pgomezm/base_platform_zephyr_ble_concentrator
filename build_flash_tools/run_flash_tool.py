'''
Flash a concentrator config.

    python build_flash_tools/run_flash_tool.py --config lora
    python build_flash_tools/run_flash_tool.py --config wifi
    python build_flash_tools/run_flash_tool.py --config lora --file output/concentrator-lora_0.1.0-dev.4f2a91c3.hex
...
def parse_args():
    parser = argparse.ArgumentParser(description="Flash a concentrator config.")
    parser.add_argument("--config", choices=sorted(BUILD_CONFIGS), default="lora",
                        help="which build to flash (default: lora)")
    parser.add_argument("--debug", action="store_true",
                        help="flash the debug build from build/<config>-debug")
...
def main() -> int:
    ...
    target = build_dir(args.config, args.debug)

    if not target.exists():
        logger.error("No build at %s. Build it first with run_build_tool.py --config %s",
                     target, args.config)
        return 1
    ...
    if args.config == "wifi" and shutil.which("esptool") is None:
